  ...
  (() => {
    ...
    const storageKey = 'trendbuzz_v42';
    ...
    function createChatNode(text, cls = '', icon = '💬', username = null, topicInfo = null) {
      ...
      node.style.left = `${6 + rand() * 26}%`;
      node.style.animationDuration = `${4.9 + rand() * 1.1}s`; 
    ...
    function spawnComment(forceTrend = false, forcedText = null, forcedClass = '', forcedIcon = null) {
      ...
      while (dom.chatLane.childNodes.length > 9) dom.chatLane.firstChild.remove();
    }
    ...
    function boardStats() {
      ...
      const overfillBodies = bodies.filter(b => (b.position.y - b.circleRadius) < 148);
      const restedOverfillBodies = overfillBodies.filter(b => b.speed < 1.45);
      const spawnZoneCount = bodies.filter(b => (b.position.y - b.circleRadius) < 112).length;
      const nearTop = Number.isFinite(maxY) ? (maxY < 144 || overfillBodies.length >= 2) : false;
      return {
        ...
        spawnZoneCount,
        pressure: clamp((content.length + hazards.length * 1.8) / 22, 0, 1),
        clutter: clamp((offTrend * 0.8 + hazards.length * 1.5) / 17, 0, 1)
      };
    }
    ...
    function dropAt(clientX) {
      ...
      if (preStats.restedOverfillCount >= 2 || preStats.spawnZoneCount >= 3) {
    ...
    function updateUi() {
      ...
      if (dom.trendCopy) {
        ...
        else if (state.trendWarning) dom.trendCopy.textContent = `あと${Math.max(1, Math.ceil(state.trendTimer))}秒で変更`;
      ...
      if (state.showCaptionTimer <= 0 && state.active) {
        ...
        if (state.clipTime > 0) {
          ...
          caption = `盤面が満杯に近い。いまは『${currentTrend().name}』だけを片付けて、上の赤ラインを越えないようにしたい。`;
        } else if (state.chainCount >= 2 && state.chainDecay > 0) {
          caption = `${state.chainCount}連鎖中。次の落下なしで続けば一気に見せ場になる。`;
    ...
    function requestLoop(ts) {
      ...
      state.overfillTime = stats.restedOverfillCount >= 1 ? state.overfillTime + dt : Math.max(0, state.overfillTime - dt * 1.9);
      ...
      const chatInterval = state.clipTime > 0 ? 0.72 : 1.12;
      ...
      if ((state.heat >= 100 || stats.pressure > 0.72 || state.topDangerTime > 0.95 || state.overfillTime > 0.32) && state.fireMode <= 0 && state.fireCooldown <= 0) {
        triggerFire(state.topDangerTime > 1.3 || state.overfillTime > 0.45 ? 'top' : 'pressure');
      }
      ...
      if (state.overfillTime > 0.46 || stats.restedOverfillCount >= 3 || stats.spawnZoneCount >= 4) {
        state.gameOverReason = 'full';
        finishGame();
      } else if (state.topDangerTime > 1.35) {
