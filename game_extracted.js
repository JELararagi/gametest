(() => {
    ...
    function contentBodyRadius(index, isHazard = false) {
      if (isHazard) return 40;
      const map = [47, 47, 46, 47, 45, 46];
      return map[index] || 46;
    }
    ...
    function syncBodyVisuals() {
      ...
      bodies.forEach(body => {
        ...
        node.style.transform = `translate3d(${(body.position.x - diameter / 2).toFixed(2)}px, ${(body.position.y - diameter / 2).toFixed(2)}px, 0) rotate(${body.angle}rad) scale(${scale})`;
        ...
        node.style.opacity = body.gameType === 'hazard' ? '1' : (trendBody ? '1' : (body.contentIndex === state.nextTrendIndex ? '0.9' : '0.76'));
        node.style.filter = body.gameType === 'hazard' ? 'none' : (trendBody ? 'saturate(1.12)' : 'saturate(.84)');
      });
    ...
    function findSafeDropX(targetX, radius) {
      ...
      const blockers = worldBodies().filter(body => (body.position.y - body.circleRadius) < Math.max(40, radius * 1.05) && body.speed < 0.55);
      if (!blockers.length) return desired;
      const isClear = (x) => blockers.every(body => Math.abs(body.position.x - x) > (body.circleRadius + radius + 2));
      if (isClear(desired)) return desired;
      const step = Math.max(10, Math.floor(radius * 0.6));
      ...
      return blockers.length >= 8 ? null : desired;
    }
    ...
    function createChatNode(text, cls = '', icon = '💬', username = null, topicInfo = null) {
      ...
      node.style.left = `${8 + rand() * 18}%`;
      node.style.animationDuration = `${2.9 + rand() * 0.45}s`;
    ...
    function spawnComment(forceTrend = false, forcedText = null, forcedClass = '', forcedIcon = null) {
      ...
      const now = performance.now();
      const cooldown = forcedClass === 'super' ? 0.72 : (forcedText ? 1.0 : 1.35);
    ...
    function boardStats() {
      ...
      const topEdges = bodies.map(b => b.position.y - b.circleRadius);
      const maxY = topEdges.length ? Math.min(...topEdges) : Number.POSITIVE_INFINITY;
      const overfillLine = 54;
      const spawnLine = 20;
      const dangerLine = 82;
      const dangerBodies = bodies.filter(b => (b.position.y - b.circleRadius) < dangerLine);
      const restedDangerBodies = dangerBodies.filter(b => b.speed < 0.6);
      const overfillBodies = bodies.filter(b => (b.position.y - b.circleRadius) < overfillLine);
      const restedOverfillBodies = overfillBodies.filter(b => b.speed < 0.45);
      const spawnZoneBodies = bodies.filter(b => (b.position.y - b.circleRadius) < spawnLine);
      const spawnZoneCount = spawnZoneBodies.filter(b => b.speed < 0.35).length;
      const nearTop = restedDangerBodies.length >= 2 || (maxY < 46 && restedDangerBodies.length >= 1);
      return {
    ...
    function initEngine() {
      engine = Engine.create({ gravity:{ x:0, y:1 }, positionIterations:6, velocityIterations:5 });
      render = null;
      updateWorldBounds();
    ...
    function dropAt(clientX) {
      ...
      const x = safeX ?? desiredX;
      const spawnY = safeX == null ? (-radius + 8) : (radius + 18);
      if (safeX == null) {
        setCaption('上が少し混んでいるので、そのまま押し込みます。', 0.7);
      }

      const idx2 = state.queue.shift();
      makeBody(x, spawnY, idx2, false);
      ...
      if (idx2 !== state.trendIndex) {
        ...
        if (rand() < 0.12) spawnComment(false, `${CONTENTS[idx2].name}置いたけど今の需要とズレたかも`, 'low', contentChatIcon(idx2));
      } else {
        state.comboTimer = Math.max(state.comboTimer, 5.2);
        if (rand() < 0.1) spawnComment(false, `${CONTENTS[idx2].name}いいね いま求めてた`, 'hot', contentChatIcon(idx2));
      }
      renderPreview();
      addParticles(x, Math.max(40, radius + 16), CONTENTS[idx2].accent, 4, 2.6);
      sfx('drop', 0.56);
    }
    ...
    function updateUi() {
      ...
      if (dom.trendCopy) {
        ...
        else dom.trendCopy.textContent = '3つで消す';
      }
    ...
    function requestLoop(ts) {
      ...
      state.overfillTime = stats.restedOverfillCount >= 3 ? state.overfillTime + dt : Math.max(0, state.overfillTime - dt * 2.6);
      ...
      const chatInterval = state.clipTime > 0 ? 3.4 : 5.2;
      ...
      if (state.overfillTime > 1.6 || stats.restedOverfillCount >= 4 || stats.spawnZoneCount >= 3) {
        state.gameOverReason = 'full';
        finishGame();
      } else if (state.topDangerTime > 2.55) {
