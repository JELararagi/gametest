  (() => {
    ...
    function showSuperchatBanner(chainTier, gain, comboCount = 1) {
      ...
      const scale = 1 + Math.min(0.52, (comboTier - 1) * 0.08);
      if (card) {
      ...
      if (amount) {
        ...
        amount.style.fontSize = `${1.02 + Math.min(0.74, (comboTier - 1) * 0.09)}rem`;
      }
      if (message) {
        message.textContent = choice(SUPERCHAT_CHAIN_MESSAGES);
        message.style.fontSize = `${1.08 + Math.min(0.54, (comboTier - 1) * 0.07)}rem`;
      }
      ...
      state.superchatBannerTimer = setTimeout(() => dom.superchatBanner?.classList.remove('show'), 1120 + Math.min(380, comboTier * 40));
      sfx('ready', Math.min(1.85, 0.84 + comboTier * 0.12));
    }
