(() => {
    ...
    const COMMENT_BANK = {"high":["いいね！","その形いい感じ！","次もいけそう！","今の消し方すき！","この流れいいよ！","うまい！","そのままいこう！","見てて楽しい！","おしい、次いける！","今のナイス！","次もそろいそう！","いい準備できてる！","その置き方わかりやすい！","スコアのびそう！","流れきてる！","その色待ってた！"],"mid":["次はどうする？","もう1こほしいね","次のお題も見よう","ゆっくりで大丈夫","形ができてきた！","まだまだいける！","次を用意しよう","ここからだよ"],"low":["ちょっと上が高いかも","今は無理しなくて大丈夫","先に消せる形を作ろう","次はやさしく置こう","山が高くなってきた","1こずつ整えよう","落ち着いていこう","今は今のお題を集めたいね"],"fire":["炎がきた！","じゃまが増えた！","近くでまとめて消そう！","まだ立て直せるよ！","落ち着いていこう！","今は安全第一！"],"warning":["次のお題を見ておこう","切りかわりの準備しよう","次に来る形を作ろう","上の並びをチェック！","次もつなげたいね"],"super":["わあ、すごい！","今の気持ちいい！","大きく消せた！","ナイスタイミング！","見てて楽しい！","今の大当たり！","そのまま連続でいこう！","すごく上手！"],"rush":["次の準備いいね！","切りかわり後が楽しみ！","先に置けてる！","その並べ方うまい！","次もすぐ消せそう！"],"jack":["先読み上手！","その形すごくいい！","次もすぐ取れそう！","きれいにつながってる！","その読みあたり！"]};
    const SPECIAL_COMMENT_BANK = {"anti":["ちょっとむずかしい形かも","次はやさしく置こう","落ち着いていこう","今は高くしすぎないでね","別の場所も見てみよう","先に安全にしよう","次で取り返せるよ","ゆっくりで大丈夫"],"superchat":["応援してるよ！","今のいいね！","そのままがんばれ！","すごく上手！","見てて楽しい！","次も見たい！","ナイスプレイ！","この流れ好き！"]};
    ...
    function noteClearActivity() {
      state.lastClearAt = performance.now();
    }
    ...
    function registerLiveBody(body) {
      ...
      const typeBucket = state.liveBodiesByType?.[body.gameType];
      if (typeBucket) liveRegistryPush(typeBucket, body, 'type');
      if (body.gameType === 'content' && Number.isInteger(body.contentIndex)) {
        const indexBucket = state.liveContentBodiesByIndex?.[body.contentIndex];
    ...
    const BOARD_BODY_ITEM_SCALE = 1.2;
    const LIGHT_MODE_SCAN_INTERVAL = 0.20;
    const LIGHT_MODE_VISUAL_INTERVAL = 1 / 30;
    const LIGHT_MODE_UI_INTERVAL = 0.18;
    const LIGHT_MODE_CLEANUP_INTERVAL = 0.42;
    const LIGHT_MODE_RESCUE_INTERVAL = 0.52;
    const LIGHT_MODE_BOARD_STATS_CACHE_MS = 260;
    const LIGHT_MODE_PARTICLE_BUDGET = 32;
    const LIGHT_MODE_PARTICLE_SCALE = 0.32;
    function contentBodyRadius(index, specialType = false) {
      if (specialType === true || specialType === 'hazard') return Math.round(31 * BOARD_BODY_ITEM_SCALE);
      if (specialType === 'fire') return Math.round(31 * BOARD_BODY_ITEM_SCALE);
      if (specialType === 'buzz') return Math.round(31 * BOARD_BODY_ITEM_SCALE);
      const map = [28, 30, 32, 34, 36, 38, 40];
      return Math.round((map[index] || 36) * BOARD_BODY_ITEM_SCALE);
    }
    ...
    const BOARD_VISUAL_ITEM_SCALE = 1.0;
    function visualDiameter(radius, specialType = false) {
      const base = radius * ((specialType === true || specialType === 'hazard' || specialType === 'fire' || specialType === 'buzz') ? 2.06 : 2.02);
    ...
    function syncBodyVisuals() {
      ...
      if (state.forceFullVisualSync) {
        ...
        for (const body of worldContentBodiesByIndex(state.trendIndex)) pushBody(body);
        for (const body of worldContentBodiesByIndex(state.nextTrendIndex)) pushBody(body);
        ...
        for (const body of bodies) {
          ...
          const moving = !body.isSleeping || body.speed > 0.03 || Math.abs(body.velocity?.x || 0) > 0.03 || Math.abs(body.velocity?.y || 0) > 0.03 || Math.abs(body.angularVelocity || 0) > 0.015;
          if (age < 380 || moving || body.outOfRangeSince || body.floatStartAt) pushBody(body);
        }
      }
      ...
      for (const body of updateBodies) {
        ...
        const className = [
          ...
          glowSet && glowSet.has(body.id) ? 'shift-glow' : ''
        ].filter(Boolean).join(' ');
        ...
        body.plugin = body.plugin || {};
        body.plugin._visualStamp = now;
        state.visualDirtyIds?.delete(body.id);
      }

      bodyVisuals.forEach((node, id) => {
        if (!state.liveBodyIds?.has(id)) {
          node.remove();
          bodyVisuals.delete(id);
          state.visualDirtyIds?.delete(id);
        }
    ...
    function updateBackgroundState(strength = 0) {
      ...
      const gameplayMode = document.body.classList.contains('gameplay-mode');
      ...
      if (dom.bgContainer) {
        dom.bgContainer.style.setProperty('--bg-base-opacity', gameplayMode ? '0' : (hype ? '0' : '0.96'));
        dom.bgContainer.style.setProperty('--bg-hype-opacity', gameplayMode ? '0' : (hype ? '1' : '0'));
      }
      if (dom.bgVideoHype) {
        dom.bgVideoHype.style.opacity = gameplayMode ? '0' : (hype ? '1' : '0');
        try {
          if (!gameplayMode && hype) {
      ...
      if (dom.bgVideo) {
        dom.bgVideo.style.opacity = gameplayMode ? '0' : (hype ? '0' : '0.96');
        try {
          if (gameplayMode || hype) {
            dom.bgVideo.pause?.();
          } else {
    ...
    function applyVoiceAvatar(kind = 'calm') {
      try {
        ...
        requestAnimationFrame(() => {
          try { setAvatarVisual(mood); } catch (_) {}
        });
      } catch (_) {}
    }
    ...
    function spawnComment(forceTrend = false, forcedText = null, forcedClass = '', forcedIcon = null) {
      ...
      if (!text) {
        ...
        if (activeEvent && noise < 0.82) {
          ...
          text = choice([
            '大バズりきた！','今まとめて伸ばして','今のうちに点をのばそう','今がチャンス','ここで見せ場！','ここで1位に近づこう', ...COMMENT_BANK.super
          ]);
          cls = 'super';
          icon = '🎬';
      ...
      if (specialType) {
        ...
        while (dom.chatLaneBack.childNodes.length > 5) dom.chatLaneBack.firstChild.remove();
      } else {
    ...
    function addParticles(x, y, color, count = 18, spread = 9) {
      const budget = Math.max(0, LIGHT_MODE_PARTICLE_BUDGET - state.particles.length);
      const safeCount = Math.max(0, Math.min(Math.round(count * LIGHT_MODE_PARTICLE_SCALE), budget));
    ...
    function boardStats(force = false) {
      const now = performance.now();
      if (!force && state.boardStatsCache && now - state.boardStatsCacheAt < LIGHT_MODE_BOARD_STATS_CACHE_MS) return state.boardStatsCache;
    ...
    function buildTouchGroupsForIndex(targetIndex) {
      const now = performance.now();
      const targetBodies = worldContentBodiesByIndex(targetIndex).filter(body => now - body.spawnAt > 220);
    ...
    function buildTouchGroupsForType(gameType, contentIndex = null) {
      ...
      const targetBodies = sourceBodies.filter(body => now - body.spawnAt > 220);
    ...
    function collectAdjacentFireBodies(seedBodies = []) {
      ...
      const fireBodies = worldBodiesByType('fire');
    ...
    function clearGroup(group) {
      ...
      if (state.clipTime > 0) spawnComment(false, '大バズり中！点が2倍！', 'super', '✨');
    ...
    function initEngine() {
      engine = Engine.create({ enableSleeping:true, gravity:{ x:0, y:1.5 }, positionIterations:5, velocityIterations:4, constraintIterations:1 });
      render = null;
      state.forceFullVisualSync = true;
      updateWorldBounds();
    ...
    function activateClipTime(reason = 'buzz_item') {
      ...
      setCaption(reason === 'buzz_item' ? 'バズりが3つつながって大バズりスタート！しばらく点が2倍。' : '大バズり中！いまは点が2倍。', 2.6);
      spawnComment(false, reason === 'buzz_item' ? 'バズり3こで大バズり！' : '大バズりつづき中！', 'super', '✨');
    ...
    function triggerFire(reason = 'bomb') {
      ...
      spawnComment(false, 'わっ、炎が広がった！', 'low', '🔥');
    ...
    function spawnHazard() {
      ...
      spawnComment(false, '爆弾きた！3こつながると炎が広がるよ！', 'low', '💣');
    ...
    function spawnBuzzItem() {
      ...
      spawnComment(false, 'バズりきた！3こで大バズり！', 'super', '✨');
    ...
    function clearAllHazards(fromBurst = false) {
      ...
      spawnComment(false, `おたすけで炎を${shuffled.length}こだけ消した！`, 'super', '🧯');
    ...
    function requestLoop(ts) {
      ...
      if (state.chainCount > 0 && state.lastClearAt > 0 && performance.now() - state.lastClearAt > 1400) {
        resetChainState();
      }
      ...
      if (state.scanTimer >= LIGHT_MODE_SCAN_INTERVAL) {
        state.scanTimer = 0;
        resolveGroups();
      }
      const previewLen = biggestPreviewLen();
      ...
      state.commentEventType = '';
      state.commentEventText = '';
      const chatInterval = state.clipTime > 0 ? 1.36 : 2.35;
      ...
      if (state.cleanupTimer >= LIGHT_MODE_CLEANUP_INTERVAL) {
        state.cleanupTimer = 0;
        cleanupOffscreenBodies();
        purgeBrokenBodies();
      }
      if (state.rescueTimer >= LIGHT_MODE_RESCUE_INTERVAL) {
        state.rescueTimer = 0;
        rescueFloatingBodies();
      }
      if (state.visualTimer >= LIGHT_MODE_VISUAL_INTERVAL) {
        updateFx(state.visualTimer);
        syncBodyVisuals();
        state.visualTimer = 0;
      }
      if (state.uiTimer >= LIGHT_MODE_UI_INTERVAL) {
...
#トレバズ`;
    ...
    dom.howtoBtn?.addEventListener('click', () => { document.body.classList.remove('title-mode','gameplay-mode'); dom.startScreen.classList.add('hidden'); dom.howtoScreen?.classList.remove('hidden'); requestAnimationFrame(alignTrendMainToGameCenter); });
    dom.howtoBackBtn?.addEventListener('click', () => { document.body.classList.remove('gameplay-mode'); document.body.classList.add('title-mode'); dom.howtoScreen?.classList.add('hidden'); dom.startScreen.classList.remove('hidden'); requestAnimationFrame(alignTrendMainToGameCenter); });
