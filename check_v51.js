  ...
  (() => {
    ...
    const storageKey = 'trendbuzz_v51';
    ...
    function createChatNode(text, cls = '', icon = '💬', username = null, topicInfo = null) {
      ...
      node.style.left = `${4 + rand() * 24}%`;
      node.style.animationDuration = `${5.2 + rand() * 0.9}s`;
      node.style.zIndex = '0';
    ...
    function spawnComment(forceTrend = false, forcedText = null, forcedClass = '', forcedIcon = null) {
      ...
      const now = performance.now();
      const cooldown = forcedClass === 'super' ? 1.05 : (forcedText ? 1.28 : 1.68);
    ...
    function requestLoop(ts) {
      ...
      const chatInterval = state.clipTime > 0 ? 5.4 : 8.2;
