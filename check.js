  ...
  (() => {
    ...
    const CONTENTS = [
      { id:'talk', name:'雑談', img:'assets/img/item_1.png', accent:'#ffffff', lines:['雑談して','近況トークちょうだい','ゆるい空気がほしい','作業しながら聞きたい'], physics:{ bboxW:512, bboxH:463, body:26, chat:'💬' } },
      { id:'react', name:'リアクション', img:'assets/img/item_2.png', accent:'#8ef8ff', lines:['反応見たい','初見リアクションほしい','びっくり顔ちょうだい','その反応で見たい'], physics:{ bboxW:512, bboxH:455, body:27, chat:'😲' } },
      { id:'cute', name:'癒し', img:'assets/img/item_3.png', accent:'#ffd95c', lines:['かわいいの見たい','癒してほしい','なごみタイムちょうだい','ゆるふわ助かる'], physics:{ bboxW:512, bboxH:429, body:28, chat:'🧸' } },
      { id:'music', name:'歌枠', img:'assets/img/item_4.png', accent:'#43eaff', lines:['歌ってほしい','声を浴びたい','耳がほしい','サビだけでもお願い'], physics:{ bboxW:511, bboxH:512, body:29, chat:'🎤' } },
      { id:'love', name:'恋バナ', img:'assets/img/item_5.png', accent:'#ff68c1', lines:['恋バナまだ？','今日は甘めで','沼らせて','惚気でも失恋でも聞きたい'], physics:{ bboxW:306, bboxH:512, body:30, chat:'💗' } },
      { id:'secret', name:'暴露', img:'assets/img/item_6.png', accent:'#ff9b42', lines:['ちょっと危ない話して','裏話まだ？','秘密トークほしい','ギリギリの話待ってる'], physics:{ bboxW:380, bboxH:512, body:31, chat:'💎' } },
      { id:'challenge', name:'限界チャレンジ', img:'assets/img/item_7.png', accent:'#90ff65', lines:['無茶企画みたい','耐久まだ？','その挑戦見届けたい','ギリギリ攻めてほしい'], physics:{ bboxW:454, bboxH:512, body:32, chat:'⚔️' } }
    ];
    ...
    const storageKey = 'trendbuzz_v53';
    ...
    function contentBodyRadius(index, isHazard = false) {
      ...
      const map = [42, 43, 44, 46, 48, 50, 52];
      return map[index] || 46;
    }
    ...
    function queueRoll() {
      ...
      if (state.trendWarning) {
        if (fever) {
          if (r < 0.34) return state.trendIndex;
          if (r < 0.97) return state.nextTrendIndex;
        } else {
        ...
        if (fever) {
          if (r < 0.78) return state.trendIndex;
          if (r < 0.94) return state.nextTrendIndex;
        } else {
    ...
    function rescueFloatingBodies() {
      ...
      for (const body of bodies) {
        ...
        if (age < 720) continue;
        if (body.position.y < 92 || body.position.y > boardHeight - 84) continue;
        const movingEnough = Math.abs(body.velocity.y) > 0.14 || Math.abs(body.velocity.x) > 0.18 || Math.abs(body.angularVelocity || 0) > 0.045;
        if (movingEnough) {
          body.floatStartAt = 0;
          continue;
        }
        ...
        for (const other of bodies) {
          ...
          const supportDx = body.circleRadius + other.circleRadius - 8;
          if (dx > supportDx || dy < 0) continue;
          if (dy <= body.circleRadius + other.circleRadius + 14) {
        ...
        if (now - body.floatStartAt < 140) continue;
        Sleeping.set(body, false);
        Body.setPosition(body, { x: clamp(body.position.x, body.circleRadius + 6, dom.board.clientWidth - body.circleRadius - 6), y: Math.min(boardHeight - body.circleRadius - 12, body.position.y + 8) });
        Body.setVelocity(body, { x: body.velocity.x * 0.3, y: Math.max(1.15, body.velocity.y + 0.45) });
