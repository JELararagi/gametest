  (() => {
    ...
    function clearBombGroup(group) {
      ...
      state.heat = clamp(state.heat + 18 + group.length * 5, 0, 100);
      state.craving = clamp(state.craving + 8 + group.length * 2, 0, 100);
      state.tension = clamp(state.tension + 10 + group.length * 2.8, 0, 100);
      state.buzz = clamp(state.buzz + 8 + group.length * 2, 0, 100);

      for (const body of group) {
        addParticles(body.position.x, body.position.y, 'rgba(255,224,120,.96)', 24, 7.4);
        addParticles(body.position.x, body.position.y, 'rgba(255,120,40,.94)', 22, 8.2);
        addParticles(body.position.x, body.position.y, 'rgba(255,90,110,.9)', 18, 6.6);
        addRing(body.position.x, body.position.y, '#ffd84c', 24, 260);
        addRing(body.position.x, body.position.y, '#ff6f3d', 34, 320);
      }
      addRing(center.x, center.y, '#ff5b73', 52, 420);
      addRing(center.x, center.y, '#ffd84c', 74, 360);
      flashScreen('#ff7b5b');
      popText(center.x, center.y, 'BOMB!!', '#fff4a3', 30);
      popText(center.x, center.y + 50, `${group.length}ヒット`, '#fff0f0', 22);
      ...
      triggerVoiceCue('fire', Math.max(2, group.length));
      say('fire', 2.2);
      return;
    }
