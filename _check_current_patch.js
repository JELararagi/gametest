  ...
  (() => {
    ...
    function noteContentClearActivity() {
      const now = performance.now();
      state.lastClearAt = now;
      state.lastContentClearAt = now;
    }
    ...
    const BOARD_BODY_ITEM_SCALE = 1.44;
    const LIGHT_MODE_SCAN_INTERVAL = 0.28;
    const LIGHT_MODE_VISUAL_INTERVAL = 1 / 45;
    const LIGHT_MODE_UI_INTERVAL = 0.28;
    const LIGHT_MODE_CLEANUP_INTERVAL = 0.82;
    const LIGHT_MODE_RESCUE_INTERVAL = 0.92;
    const LIGHT_MODE_BOARD_STATS_CACHE_MS = 520;
    const LIGHT_MODE_PARTICLE_BUDGET = 8;
    const LIGHT_MODE_PARTICLE_SCALE = 0.12;
    ...
    function updateBackgroundState(strength = 0) {
      ...
      dom.bgContainer?.classList.toggle('hype', false);
      dom.gameScreen?.classList.toggle('bigbuzz', hype);
      dom.gameShell?.classList.toggle('bigbuzz', hype);
      if (dom.bgContainer) {
        dom.bgContainer.style.setProperty('--bg-base-opacity', '0');
        dom.bgContainer.style.setProperty('--bg-hype-opacity', '0');
      }
      if (dom.bgVideoHype) {
    ...
    function setTitleBackgroundActive(active = true) {
      ...
      if (dom.bgContainer) {
        dom.bgContainer.style.setProperty('--bg-base-opacity', active ? '1' : '0');
        dom.bgContainer.style.setProperty('--bg-hype-opacity', '0');
      }
      if (dom.bgVideoHype) {
        dom.bgVideoHype.style.opacity = '0';
    ...
    function spawnComment(forceTrend = false, forcedText = null, forcedClass = '', forcedIcon = null) {
      ...
      const now = performance.now();
      const cooldown = forcedClass === 'super' ? 0.56 : (forcedText ? 0.72 : 0.86);
    ...
    function initEngine() {
      engine = Engine.create({ enableSleeping:true, gravity:{ x:0, y:1.5 }, positionIterations:4, velocityIterations:3, constraintIterations:1 });
      render = null;
      state.forceFullVisualSync = true;
      updateWorldBounds();
    ...
    function requestLoop(ts) {
      ...
      if (state.chainCount > 0 && state.lastContentClearAt > 0 && performance.now() - state.lastContentClearAt > 1400) {
        resetChainState();
      }
      ...
      state.commentEventType = '';
      state.commentEventText = '';
      const chatInterval = state.clipTime > 0 ? 1.85 : 3.10;
