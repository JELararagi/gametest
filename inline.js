...
function hasFastMovingBodies() {
      ...
      for (const body of activeBodies) {
        ...
        if ((body.speed || 0) > 0.18) return true;
        if (Math.abs(body.velocity?.y || 0) > 0.16) return true;
        if (Math.abs(body.velocity?.x || 0) > 0.12) return true;
      }
      return false;
    }
...
function syncBodyVisuals() {
    ...
    const BUILD_ID = 'v95-fall-smooth';
...
function requestLoop(ts) {
      ...
      if (state.visualTimer >= perfSettings.visualInterval || forceVisualNow) {
        ...
        if ((state.particles.length || state.rings.length || state.fxNeedsClear) && state.visualTimer >= perfSettings.visualInterval) updateFx(visualDt);
