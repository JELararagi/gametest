(() => {
    ...
    const PLAYER_LINES = {
      ...
      top:[
        '上が近い…ここで雑に置けない。','これ以上積むと危ない。','まずは盤面を開ける。','満杯ライン近い、慎重にいく。','事故る前に今消せるやつを取る。','ここで欲張ると終わる。','上が詰まってる、崩し優先。','一手ミスると配信事故。'
      ],
      switch:[
    ...
    function setAvatarVisual(key = 'normal') {
      ...
      for (const file of pool) {
        ...
        if (dom.avatar) { dom.avatar.dataset.file = file; dom.avatar.src = `assets/img/${file}`; }
        if (dom.boardCharacter && dom.boardCharacter.dataset.file !== file) {
          dom.boardCharacter.dataset.file = file;
          dom.boardCharacter.src = `assets/img/${file}`;
        }
        return;
      }
    ...
    function renderPreview() {
      ...
      const previewTop = Math.max(8, metrics.fullLineY - Math.round(size / 2) - 6);
      dom.dropGuide.style.left = `${x}px`;
      dom.previewItem.style.left = `${x}px`;
      dom.previewRing.style.left = `${x}px`;
      dom.previewItem.style.top = `${previewTop + 5}px`;
      dom.previewRing.style.top = `${previewTop}px`;
      dom.previewItem.style.width = `${Math.max(48, Math.round(size * 0.82))}px`;
      dom.previewItem.style.height = `${Math.max(48, Math.round(size * 0.82))}px`;
      dom.previewRing.style.width = `${Math.round(size + 12)}px`;
      dom.previewRing.style.height = `${Math.round(size + 12)}px`;
      refreshQueue();
    }
    ...
    function clearGroup(group) {
      ...
      if (state.buzz >= 100 && !state.buzzReady && state.buzzMode <= 0) {
        ...
        dom.burstBtn.innerHTML = '<div><div class="icon">🧯</div><div class="txt">火消しする</div></div>';
    ...
    function activateClipTime() {
      ...
      say('big', 2.1);
      flashScreen('#ffe37a');
      popText(dom.board.clientWidth / 2, dom.board.clientHeight * 0.33, '大バズり!!', '#fff0a3', 34);
    ...
    function activateBurst() {
      ...
      dom.burstBtn.innerHTML = '<div><div class="icon">🧯</div><div class="txt">火消し中!</div></div>';
    ...
    function finishGame() {
      ...
      triggerVoiceCue((crownTargetScore(state.seed) > 0 && Math.floor(state.score) >= crownTargetScore(state.seed)) ? 'win' : 'lose');
      ...
      setTimeout(() => {
        ...
        const resultStinger = new Audio(winLike ? 'assets/audio/bgm_result_win.wav' : 'assets/audio/bgm_result_lose.wav');
        resultStinger.volume = muted ? 0 : 0.62;
        resultStinger.play().catch(() => {});
