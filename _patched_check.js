  ...
  (() => {
    ...
    function setAvatarVisual(key = 'normal') {
      ...
      for (const file of pool) {
        ...
        if (dom.avatar && dom.avatar.dataset.file !== file) {
          dom.avatar.decoding = 'sync';
          dom.avatar.dataset.file = file;
          dom.avatar.src = nextSrc;
        }
        if (dom.boardCharacter && dom.boardCharacter.dataset.file !== file) {
          dom.boardCharacter.decoding = 'sync';
          dom.boardCharacter.dataset.file = file;
          dom.boardCharacter.src = nextSrc;
        }
        return;
      }
      if (dom.avatar) { dom.avatar.dataset.file = 'char_normal.png'; dom.avatar.src = 'assets/img/char_normal.png'; }
      if (dom.boardCharacter) { dom.boardCharacter.dataset.file = 'char_normal.png'; dom.boardCharacter.src = 'assets/img/char_normal.png'; }
    }
    ...
    function syncBodyVisuals() {
      ...
      bodies.forEach(body => {
        ...
        const drawX = clamp(body.position.x, -diameter * 0.35, boardW - diameter * 0.65);
        const drawY = clamp(body.position.y, -diameter * 0.35, boardH - diameter * 0.65);
        ...
        const zValue = String(100 + Math.round(body.position.y));
        ...
        const filterValue = body.gameType === 'hazard' ? 'none' : (trendBody ? 'saturate(1.1)' : 'none');
    ...
    function applyAppHeight() {
      ...
      const stageScale = Math.min(viewportWidth / BASE_STAGE_W, viewportHeight / BASE_STAGE_H) || 1;
    ...
    function applyVoiceSubtitle(path, hold = 3.8) {
      ...
      state.voiceSubtitleTimer = Math.max(state.voiceSubtitleTimer || 0, hold);
      setMoodLabel(line);
      return true;
    }
    ...
    function updateBackgroundState(strength = 0) {
      ...
      if (dom.bgVideoHype) dom.bgVideoHype.style.opacity = hype ? '0.82' : '0';
      if (dom.bgVideo) dom.bgVideo.style.opacity = hype ? '0.28' : '0.62';
    }
    ...
    function triggerVoiceCue(kind, tier = 1) {
      const now = performance.now();
      const cooldownMap = { clear:620, chain:980, hype:1200, warning:1400, fire:1500, apology_ready:1200, apology_fire:1400, rush:1100, jack:1200, big:1200, focus:900, top:1100, win:1400, lose:1400, default:900 };
      ...
      const chosen = variants.length ? choiceGame(variants) : '';
      let played = false;
      if (chosen) {
    ...
    function updateMoodText(dt = 0) {
      ...
      if (state.lastVoiceText) {
        setMoodLabel(state.lastVoiceText);
        return;
      }
    ...
    function setTrend(index, withFlash = false, previousIndex = null) {
      ...
      dom.trendCopy.textContent = 'あと3消し！';
      ...
      setCaption(`いまは『${trend.name}』を3つ以上で消す。消えたら次は『${nextTrend().name}』。さらにその次は『${futureTrend().name}』。`, 2.8);
      spawnComment(true, null, 'hot');
      say(withFlash ? 'switch' : 'focus', 2.4);
      triggerVoiceCue(withFlash ? 'rush' : 'focus');
    ...
    function ensureQueueLength(min = 3) {
      while (state.queue.length < min) state.queue.push(queueRoll());
    }

    function refreshQueue() {
      ensureQueueLength(3);
      const q0 = CONTENTS[state.queue[0] ?? 0] || CONTENTS[0];
      const q1 = CONTENTS[state.queue[1] ?? state.queue[0] ?? 0] || CONTENTS[0];
      const q2 = CONTENTS[state.queue[2] ?? state.queue[1] ?? state.queue[0] ?? 0] || CONTENTS[0];
      dom.previewItem.src = q0.img;
      dom.previewItem.alt = q0.name;
      dom.nextMini1.src = q1.img;
      dom.nextMini1.alt = `next ${q1.name}`;
      dom.nextMini2.src = q2.img;
      dom.nextMini2.alt = `after next ${q2.name}`;
    }
    ...
    function renderPreview() {
      ...
      const previewTop = Math.max(8, metrics.fullLineY - ringSize - 4);
    ...
    function makeBody(x, y, index, isHazard = false) {
      ...
      const node = ensureBodyVisual(body);
      if (node) {
        const diameter = visualDiameter(radius, isHazard);
        node.dataset.diameter = String(diameter);
        node.style.width = `${diameter}px`;
        node.style.height = `${diameter}px`;
        node.style.transform = `translate3d(${Math.round((x - diameter / 2) * 10) / 10}px, ${Math.round((y - diameter / 2) * 10) / 10}px, 0)`;
    ...
    function cleanupOffscreenBodies() {
      ...
      for (const body of bodies) {
        ...
        if (body.position.y < body.circleRadius * 0.2) {
          ...
          Body.setPosition(body, { x: body.circleRadius + 8, y: clamp(body.position.y, body.circleRadius + 14, boardH - body.circleRadius - 8) });
          Body.setVelocity(body, { x: Math.max(0, Math.abs(body.velocity.x) * 0.35), y: body.velocity.y });
        } else if (body.position.x > boardW + body.circleRadius) {
          Body.setPosition(body, { x: boardW - body.circleRadius - 8, y: clamp(body.position.y, body.circleRadius + 14, boardH - body.circleRadius - 8) });
          Body.setVelocity(body, { x: -Math.max(0, Math.abs(body.velocity.x) * 0.35), y: body.velocity.y });
        }
      }
    }
    ...
    function updateWorldBounds() {
      ...
      wallBodies = [
        Bodies.rectangle(boardW / 2, boardH + 34, boardW + 80, 68, { isStatic:true, render:{ visible:false } }),
        Bodies.rectangle(-32, boardH / 2, 64, boardH + 80, { isStatic:true, render:{ visible:false } }),
        Bodies.rectangle(boardW + 32, boardH / 2, 64, boardH + 80, { isStatic:true, render:{ visible:false } })
      ];
    ...
    function clearGroup(group) {
      ...
      state.score += gain;
      const timeBonus = group.length >= 3 ? (state.clipTime > 0 ? 20 : 10) : 0;
      if (timeBonus > 0) state.streamTime = Math.min(state.streamTotal + 45, state.streamTime + timeBonus);
      ...
      if (timeBonus > 0) popText(center.x, center.y - 84, `TIME +${timeBonus}s`, '#8ef8ff', 20);
      ...
      if (state.buzz >= 100 && !state.buzzReady && state.buzzMode <= 0) {
        ...
        dom.burstBtn.innerHTML = '<div><div class="icon">🧯</div><div class="txt">火消をする</div></div>'; flashBurstButton();
        setCaption('謝罪配信の準備完了。押すと今ある炎上をまとめて消せる。危ない時の立て直しに使いたい。', 2.4);
        spawnComment(false, '今なら火消しできる', 'super', '🧯');
        triggerVoiceCue('apology_ready');
        sfx('ready', 1.1);
      }
    ...
    function resetState() {
      ...
      state.queue = [queueRoll(), queueRoll(), queueRoll()];
      state.previewGroups = [];
      state.nextPreviewGroups = [];
      state.lastTs = 0;
    ...
    function activateClipTime() {
      ...
      setCaption('大バズり中！ いまが最大の見せ場。いまのトレンドをまとめて取って一気に伸ばそう。', 2.6);
      spawnComment(false, '大バズりきた！', 'super', '🎬');
      ...
      setHyperMix(1);
      addParticles(dom.board.clientWidth / 2, dom.board.clientHeight * 0.42, 'rgba(255,216,76,.96)', 32, 9);
      addParticles(dom.board.clientWidth / 2, dom.board.clientHeight * 0.42, 'rgba(255,47,146,.9)', 20, 8);
    ...
    function startGame() {
      ...
      dom.pauseOverlay?.classList.add('hidden');
      if (dom.avatar) dom.avatar.src = 'assets/img/char_normal.png';
      if (dom.boardCharacter) dom.boardCharacter.src = 'assets/img/char_normal.png';
    ...
    function activateBurst() {
      ...
      dom.burstBtn.innerHTML = '<div><div class="icon">🧯</div><div class="txt">謝罪配信</div></div>';
    ...
    function clearAllHazards(fromBurst = false) {
      ...
      for (const body of hazards) {
        ...
        popText(body.position.x, body.position.y - 10, '火消し!', '#fff0a3', 18);
      }
      ...
      spawnComment(false, '謝罪配信で立て直した！', 'super', '🧯');
      setCaption('謝罪配信で火消し成功。いま出ていた炎上を全部消した。ここから立て直せる。', 2.2);
    ...
    function updateUi() {
      ...
      dom.trendTimer.textContent = state.overfillTime > 0.18 ? '満杯注意' : 'あと3消し！';
      if (dom.trendProgressFill) dom.trendProgressFill.style.width = '100%';
      if (dom.nextSwitchCopy) dom.nextSwitchCopy.textContent = ''; 
      if (dom.trendCopy) {
        if (state.overfillTime > 0.18) dom.trendCopy.textContent = '満杯注意';
        else dom.trendCopy.textContent = 'あと3消し！';
      }
...
#トレバズ`;
    ...
    dom.howtoBtn?.addEventListener('click', () => { dom.startScreen.classList.add('hidden'); dom.howtoScreen?.classList.remove('hidden'); });
    dom.howtoBackBtn?.addEventListener('click', () => { dom.howtoScreen?.classList.add('hidden'); dom.startScreen.classList.remove('hidden'); });
