  ...
  (() => {
    ...
    const COMMENT_BANK = {
      ...
      bigbuzz:[
        '大バズり中！いまは点が2倍！','いまがまとめ取りチャンス！','いまのうちにどんどん消したい！','この時間に点をのばそう！','大バズりが続いてる！'
      ],
      rescue:[
      ...
      buzzPrep:[
        'バズりが2こある！あと1こで大バズり！','バズりをもう1こ引けたら大チャンス！','バズり3こで点が2倍になるよ','バズりをそろえたい！','バズりが来たらまとめて取りたい！'
      ],
      chain:[
...
function refreshActiveBodies(now = performance.now()) {
  ...
  for (let i = 0; i < liveBodies.length; i += 1) {
    ...
    const moving = !body.isSleeping
      || body.speed > 0.04
      || Math.abs(body.velocity?.x || 0) > 0.04
      || Math.abs(body.velocity?.y || 0) > 0.04
      || Math.abs(body.angularVelocity || 0) > 0.02;
    if (moving) body.plugin.lastActiveAt = now;
    const recentlyActive = moving || (body.plugin.lastActiveAt && now - body.plugin.lastActiveAt < 320);
    if (!recentlyActive) continue;
    nextBodies.push(body);
    nextIds.add(body.id);
    if (moving) markGroupScanDirty(body);
  }
...
function collectRepairCandidates() {
  const map = new Map();
  getActiveBodies().forEach(body => {
    if (body && state.liveBodyIds?.has(body.id) && !body.plugin?.pendingRemoval) map.set(body.id, body);
  });
  state.repairQueueIds?.forEach(id => {
    const body = bodyById(id);
    if (body && !body.plugin?.pendingRemoval) map.set(body.id, body);
  });
...
function unregisterLiveBody(body) {
      ...
      if (Array.isArray(state.activeBodies)) state.activeBodies = state.activeBodies.filter(item => item?.id !== body.id);
    ...
    function updateVisualDirtyContext(previewMap, nextPreviewMap, glowSet) {
      ...
      const previewIds = [...previewMap.keys()];
      const nextPreviewIds = [...nextPreviewMap.keys()];
      const glowIds = glowSet ? [...glowSet] : [];
      if ((state.visualPreviewIds || []).join(',') !== previewIds.join(',')) {
        markBodiesVisualDirty((state.visualPreviewIds || []).map(bodyById).filter(Boolean));
        markBodiesVisualDirty(previewIds.map(bodyById).filter(Boolean));
        state.visualPreviewIds = previewIds;
      }
      if ((state.visualNextPreviewIds || []).join(',') !== nextPreviewIds.join(',')) {
        markBodiesVisualDirty((state.visualNextPreviewIds || []).map(bodyById).filter(Boolean));
        markBodiesVisualDirty(nextPreviewIds.map(bodyById).filter(Boolean));
        state.visualNextPreviewIds = nextPreviewIds;
      }
      if ((state.visualGlowIds || []).join(',') !== glowIds.join(',')) {
        markBodiesVisualDirty((state.visualGlowIds || []).map(bodyById).filter(Boolean));
        markBodiesVisualDirty(glowIds.map(bodyById).filter(Boolean));
        state.visualGlowIds = glowIds;
      }
    }
...
const PERF_PRESET_DEFAULT = Object.freeze({
      ...
      visualInterval:1 / 45,
      uiInterval:0.58,
      cleanupInterval:1.55,
      rescueInterval:1.72,
      boardStatsCacheMs:1450,
      particleBudget:16,
      particleScale:0.32,
      particleFadeRate:1.85,
      particleLife:0.96,
      particleSizeScale:1.04,
      ringBudget:12,
      minorRingThreshold:0,
      majorRingThreshold:92,
      popTextMax:6,
      popTextMinGapMs:40,
      popTextScale:1.06,
      commentCooldownScale:1.08,
      commentIntervalScale:1.18,
      commentDropChance:0.02,
      commentLaneMax:3,
      specialCommentLaneMax:3,
      superchatMinGapMs:900,
      videoPreload:'metadata',
      fxResolutionScale:0.44,
      transformPositionThreshold:0.68,
      transformAngleThreshold:0.04,
      motionThreshold:0.07,
      motionAngularThreshold:0.03,
      newBodyVisualMs:220,
      backgroundWatchdogInterval:1.6,
      bgmNormalVolume:0.28,
      bgmHyperVolume:0.34
    });

    const PERF_PRESET_MOBILE = Object.freeze({
      ...PERF_PRESET_DEFAULT
    });
    ...
    function applyMediaProfile() {
      ...
      applyElementSource(dom.bgVideoHype, resolveVideoAsset(dom.bgVideoHype), 'metadata');
      [dom.bgmNormal, dom.bgmHyper, dom.audioMerge, dom.voiceStart, dom.voiceFire, dom.voiceWin, dom.voiceLose].forEach(audio => {
        if (!audio) return;
        const desired = resolveAudioAsset(audio.getAttribute('src') || audio.dataset.assignedSrc || '');
        if (desired) applyElementSource(audio, desired, 'metadata');
      });
    }
...
function applyPerformanceProfile() {
      ...
      perfSettings = next;
      document.body.classList.remove('mobile-bg-lite');
      applyMediaProfile();
    ...
    const BOARD_VISUAL_ITEM_SCALE = 0.9;
...
function hasPendingVisualWork() {
      ...
      if (state.visualDirtyIds?.size || state.visualPreviewIds?.size || state.visualNextPreviewIds?.size || state.visualGlowIds?.size) return true;
      return !!(state.activeBodies && state.activeBodies.length);
    }
...
function syncBodyVisuals() {
      ...
      updateVisualDirtyContext(previewMap, nextPreviewMap, glowSet);

      const updateBodies = [];
      const queuedIds = new Set();
      const pushBody = (body) => {
        if (!body || queuedIds.has(body.id) || !state.liveBodyIds?.has(body.id)) return;
        queuedIds.add(body.id);
      ...
      if (state.forceFullVisualSync) {
        ...
        const activeBodies = state.activeBodies || [];
        for (const body of activeBodies) {
          const age = now - (body.spawnAt || 0);
          const moving = !body.isSleeping || body.speed > perfSettings.motionThreshold || Math.abs(body.velocity?.x || 0) > perfSettings.motionThreshold || Math.abs(body.velocity?.y || 0) > perfSettings.motionThreshold || Math.abs(body.angularVelocity || 0) > perfSettings.motionAngularThreshold;
          if (age < perfSettings.newBodyVisualMs || moving || body.outOfRangeSince || body.floatStartAt) pushBody(body);
        }
      }
      ...
      for (const body of updateBodies) {
        ...
        const rawX = Number.isFinite(body.position.x) ? body.position.x : boardW / 2;
        const rawY = Number.isFinite(body.position.y) ? body.position.y : boardH * 0.5;
        const rawAngle = Number.isFinite(body.angle) ? body.angle : 0;
    ...
    const BUILD_ID = 'v94-drop-preview-overlay-fix';
    ...
    function applyAppHeight(force = false) {
      ...
      if (resized || force) alignTrendMainToGameCenter();
    }
    ...
    function primeBigBuzzVisuals() {
      if (!dom.boardBgVideoHype) return;
      applyElementSource(dom.boardBgVideoHype, resolveVideoAsset(dom.boardBgVideoHype), 'auto');
    ...
    function backgroundWatchdog(dt = 0) {
      ...
      const mode = currentBackgroundMode();
      if (mode === 'bigbuzz' && dom.boardBgVideoHype?.paused) {
        playLoopVideo(dom.boardBgVideoHype, { preload:'auto' });
      }
    }
    ...
    function showBigBuzzBanner(text = '大バズリ!!', sub = 'ポイント×２') {
    ...
    function showSuperchatBanner(chainTier, gain, comboCount = 1) {
      ...
      const now = performance.now();
      if (now - lastSuperchatBannerAt < (perfSettings.superchatMinGapMs || 0)) return;
      ...
      state.superchatBannerTimer = setTimeout(() => dom.superchatBanner?.classList.remove('show'), (4000 + tier * 520 + comboTier * 150) * 2);
...
function makeBody(x, y, index, specialType = false) {
      ...
      body.sleepThreshold = isBomb ? 28 : (isFire ? 26 : (isBuzz ? 24 : 20));
      body.plugin = body.plugin || {};
    ...
    function boardStats(force = false) {
      const now = performance.now();
      if (!force && state.boardStatsCache && now - state.boardStatsCacheAt < perfSettings.boardStatsCacheMs) return state.boardStatsCache;
...
function rescueFloatingBodies() {
      ...
      const now = performance.now();
      const hash = buildSpatialHash(worldBodies());
      for (const body of bodies) {
        const age = now - (body.spawnAt || 0);
        if (age < 420) continue;
        ...
        visitNearbyFromHash(hash, body, 1, other => {
          if (other.id === body.id || other.plugin?.pendingRemoval) return false;
        ...
        if (now - body.floatStartAt < 110) continue;
        requestHighPrecisionPhysics(260);
...
function buildTouchGroupsNearSeeds(sourceBodies, seedBodies, pad = 0.5) {
  ...
  const sourceMap = new Map(sourceBodies.map(body => [body.id, body]));
  const hash = buildSpatialHash(sourceBodies);
  const emitted = new Set();
  const groups = [];
  for (let i = 0; i < seedBodies.length; i += 1) {
    const root = sourceMap.get(seedBodies[i]?.id);
    if (!root || emitted.has(root.id)) continue;
    const stack = [root];
    const localVisited = new Set([root.id]);
    const group = [];
    while (stack.length) {
      const current = stack.pop();
      group.push(current);
      ...
      visitNearbyFromHash(hash, current, 1, other => {
        if (other.id === current.id || localVisited.has(other.id)) return false;
        if (!touchingWithPad(current, other, pad)) return false;
...
function buildTouchGroupsForIndex(targetIndex, seedBodies = null) {
      const now = performance.now();
      const targetBodies = worldContentBodiesByIndex(targetIndex).filter(body => state.liveBodyIds?.has(body.id) && !body.plugin?.pendingRemoval && now - body.spawnAt > 220);
      if (!Array.isArray(seedBodies) || !seedBodies.length) return buildTouchGroupsFromBodies(targetBodies, 0.5);
      const relevantSeeds = seedBodies.filter(body => body?.gameType === 'content' && body.contentIndex === targetIndex && state.liveBodyIds?.has(body.id) && !body.plugin?.pendingRemoval && now - body.spawnAt > 120);
      if (!relevantSeeds.length) return [];
      return buildTouchGroupsNearSeeds(targetBodies, relevantSeeds, 0.5);
    }
...
function buildTouchGroupsForType(gameType, contentIndex = null, seedBodies = null) {
      ...
      const targetBodies = sourceBodies.filter(body => state.liveBodyIds?.has(body.id) && !body.plugin?.pendingRemoval && now - body.spawnAt > 220);
      ...
      const relevantSeeds = seedBodies.filter(body => body?.gameType === gameType && state.liveBodyIds?.has(body.id) && !body.plugin?.pendingRemoval && now - body.spawnAt > 120);
      if (!relevantSeeds.length) return [];
      return buildTouchGroupsNearSeeds(targetBodies, relevantSeeds, loosePad);
    }
...
function scanAllClearableGroups() {
      const now = performance.now();
      const dirtyIds = state.groupScanDirtyIds instanceof Set ? Array.from(state.groupScanDirtyIds) : [];
      const active = getActiveBodies();
      const seedMap = new Map();
      for (let i = 0; i < dirtyIds.length; i += 1) {
        const body = bodyById(dirtyIds[i]);
        if (body && !body.plugin?.pendingRemoval) seedMap.set(body.id, body);
      }
      for (let i = 0; i < active.length; i += 1) {
        const body = active[i];
        if (body && !body.plugin?.pendingRemoval) seedMap.set(body.id, body);
      }
      const seedBodies = Array.from(seedMap.values());
      const periodicFullScan = now - (state.lastFullGroupScanAt || 0) > 2.4 * 1000;
      const tooManySeeds = seedBodies.length >= Math.max(12, (state.liveBodies?.length || 0) * 0.45);
      const groups = [];
      if (!seedBodies.length && !periodicFullScan) {
        return groups;
      }
      if (!seedBodies.length || periodicFullScan || tooManySeeds) {
    ...
    function collectAdjacentFireBodies(seedBodies = []) {
      ...
      const allowFireCascade = seedBodies.some(body => {
        const type = body?.gameType || 'content';
        return type !== 'hazard' && type !== 'buzz';
      });
      if (!allowFireCascade) return [];
      const fireBodies = worldBodiesByType('fire').filter(body => state.liveBodyIds?.has(body.id) && !body.plugin?.pendingRemoval);
    ...
    function clearBombGroup(group) {
      ...
      if (adjacentFire.length) popText(center.x, center.y + 64, `炎まきこみ x${adjacentFire.length}`, '#ffb7c5', 18);
      spawnComment(false, `爆弾が${group.length}こつながって大炎上！`, 'low', '💣');
      triggerFire('bomb_chain');
      const fireCount = 3 + Math.floor(rand() * 3);
    ...
    function clearBuzzGroup(group) {
      ...
      if (adjacentFire.length) popText(center.x, center.y + 96, `炎まきこみ x${adjacentFire.length}`, '#ffb7c5', 18);
      return gain;
    }
    ...
    function clearGroup(group) {
      ...
      if (adjacentFire.length) {
        ...
        popText(center.x, center.y + 66, `炎まきこみ x${adjacentFire.length}`, '#ffb7c5', 18);
      }
      ...
      if (state.clipTime > 0) spawnComment(false, '大バズり中！いまは点が2倍！', 'super', '✨');
      ...
      if (chainTier >= 4) {
        ...
        spawnComment(false, '連鎖つづいてる！', 'super', '💥');
...
function initEngine() {
    ...
    function activateClipTime(reason = 'buzz_item') {
      ...
      setCaption(reason === 'buzz_item' ? 'バズりが3つつながって大バズリスタート！しばらくポイントが2倍。' : '大バズリ中！いまはポイントが2倍。', 2.6);
      spawnComment(false, reason === 'buzz_item' ? 'バズり3こで大バズリ！' : '大バズリつづき中！', 'super', '✨');
      triggerVoiceCue('hype', 2, { force:fresh });
      flashScreen('#ffe37a');
      showBigBuzzBanner('大バズリ!!', 'ポイント×２');
      setHyperMix(1);
      updateBackgroundState(true);
      ...
      playAssetSfx('big', fresh ? 0.94 : 0.82);
      sfx('big', fresh ? 1.2 : 0.96);
    }
    ...
    function triggerFire(reason = 'bomb') {
      ...
      setCaption(reason === 'bomb_chain' ? '爆弾が3こつながって大炎上。炎アイテムが落ちてくる。' : '大炎上中。炎を巻きこみながら立て直そう。', 2.2);
    ...
    function clearAllHazards(fromBurst = false) {
      ...
      if (!hazards.length) {
        if (fromBurst) {
          setCaption('今いっしょに消せる炎はないみたい。', 1.4);
          sfx('warn', 0.72);
        }
        return 0;
      }
    ...
    function updateUi() {
      ...
      if (state.showCaptionTimer <= 0 && state.active) {
        let caption = '';
        if (state.clipTime > 0) {
          caption = `大バズり中。『${currentTrend().name}』を取ると点が2倍。`;
...
function requestLoop(ts) {
      ...
      if (state.visualTimer >= perfSettings.visualInterval) {
        const visualDt = state.visualTimer;
      ...
      if (state.scanTimer >= perfSettings.scanInterval) {
        state.scanTimer = 0;
        resolveGroups();
      }
      const previewLen = biggestPreviewLen();
      ...
      state.commentEventType = '';
      state.commentEventText = '';
      const chatInterval = (state.clipTime > 0 ? 1.85 : 3.10) * perfSettings.commentIntervalScale;
      ...
      if (state.cleanupTimer >= perfSettings.cleanupInterval) {
        state.cleanupTimer = 0;
        cleanupOffscreenBodies();
        purgeBrokenBodies();
      }
      if (state.rescueTimer >= perfSettings.rescueInterval) {
        state.rescueTimer = 0;
        rescueFloatingBodies();
      }
      if (state.uiTimer >= perfSettings.uiInterval) {
    ...
    function finishGame() {
      ...
      setTimeout(() => {
        ...
        stopResultBgm();
        resultStingerAudio = new Audio(resolveAudioAsset(winLike ? 'assets/audio/bgm_result_win.wav' : 'assets/audio/bgm_result_lose.wav'));
...
#トレバズ`;
  ...
  window.addEventListener('load', () => {
    ...
    dom.howtoBtn?.addEventListener('click', () => { document.body.classList.remove('title-mode','gameplay-mode'); setTitleBackgroundActive(false); dom.startScreen.classList.add('hidden'); dom.howtoScreen?.classList.remove('hidden'); requestAnimationFrame(alignTrendMainToGameCenter); });
    dom.howtoBackBtn?.addEventListener('click', () => { document.body.classList.remove('gameplay-mode'); document.body.classList.add('title-mode'); setTitleBackgroundActive(true); dom.howtoScreen?.classList.add('hidden'); dom.startScreen.classList.remove('hidden'); requestAnimationFrame(alignTrendMainToGameCenter); });
    dom.howtoStartBtn?.addEventListener('click', startGame);
    dom.pauseBtn?.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); togglePause(); });
    dom.resumeGameBtn?.addEventListener('click', () => togglePause(false));
