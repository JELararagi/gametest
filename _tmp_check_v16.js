  ...
  (() => {
    ...
    function updateBackgroundState(strength = 0) {
      ...
      if (dom.bgContainer) {
        dom.bgContainer.style.setProperty('--bg-base-opacity', hype ? '0.94' : '0.96');
        dom.bgContainer.style.setProperty('--bg-hype-opacity', hype ? '0.72' : '0');
      }
      if (dom.bgVideoHype) {
        dom.bgVideoHype.style.opacity = hype ? '0.72' : '0';
        try { if (hype) dom.bgVideoHype.play?.(); else { dom.bgVideoHype.pause?.(); dom.bgVideoHype.currentTime = 0; } } catch (_) {}
      }
      if (dom.bgVideo) dom.bgVideo.style.opacity = hype ? '0.94' : '0.96';
    }
    ...
    function showSuperchatBanner(chainTier, gain, comboCount = 1) {
      ...
      const scale = 0.78 + Math.min(0.84, (tier - 1) * 0.12) + Math.min(0.18, (comboTier - 1) * 0.02);
      if (card) {
        card.style.borderColor = theme.border;
        card.style.boxShadow = `0 0 0 1px ${theme.border}44, 0 18px 44px rgba(0,0,0,.34), 0 0 ${24 + comboTier * 4}px ${theme.fill}`;
        card.style.transform = `scale(${scale})`;
      }
      if (amount) {
        amount.textContent = `+${fmt(Math.max(0, Math.round(gain || 0)))}`;
        amount.style.color = theme.border;
        amount.style.fontSize = `${0.82 + Math.min(1.02, (tier - 1) * 0.14) + Math.min(0.22, (comboTier - 1) * 0.02)}rem`;
      }
      if (message) {
        message.textContent = choice(SUPERCHAT_CHAIN_MESSAGES);
        message.style.fontSize = `${0.88 + Math.min(1.12, (tier - 1) * 0.16) + Math.min(0.22, (comboTier - 1) * 0.02)}rem`;
      }
      ...
      state.superchatBannerTimer = setTimeout(() => dom.superchatBanner?.classList.remove('show'), 1900 + tier * 220 + comboTier * 60);
    ...
    function buildTouchGroupsForType(gameType, contentIndex = null) {
      const targetBodies = worldBodies().filter(body => {
        ...
        return performance.now() - body.spawnAt > 220;
      });
      ...
      for (const body of targetBodies) {
        ...
        while (stack.length) {
          ...
          for (const other of targetBodies) {
            if (visited.has(other.id) || other.id === current.id) continue;
            if (touchingWithPad(current, other, loosePad)) {
              visited.add(other.id);
              stack.push(other);
            }
          }
    ...
    function clearBombGroup(group) {
      ...
      sfx('burst', 1.5);
      const gain = applyComboGain({ center, accent:'#ffd84c', chainTier, comboCount:state.combo, noteText:'爆弾3連結!' });
    ...
    function clearBuzzGroup(group) {
      ...
      sfx('big', 1.46);
      const gain = applyComboGain({ center, accent:'#ffe37a', chainTier, comboCount:state.combo, noteText:'バズり3連結!' });
      popText(center.x, center.y + 56, '大バズり突入!!', '#fff4a3', 26);
      spawnComment(false, 'バズり3連結！ 大バズり発動！', 'super', '✨');
      triggerVoiceCue('hype', Math.max(2, chainTier), { force:true });
    ...
    function triggerFire(reason = 'bomb') {
      ...
      setCaption(reason === 'bomb_chain' ? '爆弾が3つつながって大炎上。謝罪配信で炎上だけを消せる。' : '大炎上中。謝罪配信で炎上だけを消せる。', 2.2);
    ...
    function updateUi() {
      ...
      let eventMiniText = '';
      if (isFeverActive()) eventMiniText = `大バズり ${Math.max(1, Math.ceil(state.clipTime))}s`;
