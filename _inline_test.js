(() => {
    ...
    function swapBoardCharacter(file, nextSrc) {
      ...
      if (back) {
        ...
        if (back.src !== nextSrc) back.src = nextSrc;
      }
    ...
    function syncBodyVisuals() {
      ...
      bodies.forEach(body => {
        ...
        const drawX = clamp(body.position.x, diameter / 2, boardW - diameter / 2);
        const drawY = clamp(body.position.y, diameter / 2, boardH - diameter / 2);
    ...
    function alignTrendMainToGameCenter() {
      if (!dom.gameShell || !dom.centerTop || !dom.trendMain) return;
      const shellRect = dom.gameShell.getBoundingClientRect();
      const hudRect = dom.centerTop.getBoundingClientRect();
      if (!shellRect.width || !hudRect.width) return;
      const centerXInHud = shellRect.left + (shellRect.width / 2) - hudRect.left;
      dom.trendMain.style.left = `${Math.round(centerXInHud * 10) / 10}px`;
    ...
    function showSpeechBubble(text, hold = 3) {
    ...
    function setMoodLabel(text) {
      state.speechBubbleText = normalizeSpeechText(text);
      refreshSpeechBubble();
    }
    ...
    function say(mode, hold = 2.4) {
      ...
      showSpeechBubble(state.currentLine, hold);
    }
    ...
    function createChatNode(text, cls = '', icon = '💬', username = null, topicInfo = null) {
      ...
      node.style.animationDuration = `${6.4 + rand() * 1.7}s`;
    ...
    function spawnComment(forceTrend = false, forcedText = null, forcedClass = '', forcedIcon = null) {
      ...
      while (dom.chatLane.childNodes.length > 16) dom.chatLane.firstChild.remove();
    }
    ...
    function updateWorldBounds() {
      ...
      wallBodies = [
        Bodies.rectangle(boardW / 2, boardH + 24, boardW + 120, 80, { isStatic:true, render:{ visible:false } }),
        Bodies.rectangle(-26, boardH / 2, 72, boardH + 120, { isStatic:true, render:{ visible:false } }),
        Bodies.rectangle(boardW + 26, boardH / 2, 72, boardH + 120, { isStatic:true, render:{ visible:false } })
      ];
    ...
    function clearGroup(group) {
      ...
      if (state.buzz >= 100 && !state.buzzReady && state.buzzMode <= 0) {
        ...
        setBurstButtonLabel('ready'); flashBurstButton();
    ...
    function requestLoop(ts) {
      ...
      const chatInterval = state.clipTime > 0 ? 1.15 : 1.75;
