(() => {
    ...
    function createChatNode(text, cls = '', icon = '💬', username = null, topicInfo = null) {
      ...
      node.style.animationDuration = `${3.2 + rand() * 0.35}s`;
    ...
    function boardStats() {
      ...
      const now = performance.now();
      const settledBodies = bodies.filter(b => now - (b.spawnAt || 0) > 1100);
      const topEdges = settledBodies.map(b => b.position.y - b.circleRadius);
      const maxY = topEdges.length ? Math.min(...topEdges) : Number.POSITIVE_INFINITY;
      const { fullLineY, dangerLineY } = getBoardMetrics();
      const dangerBodies = settledBodies.filter(b => (b.position.y - b.circleRadius) < dangerLineY);
      const restedDangerBodies = dangerBodies.filter(b => b.speed < 0.5);
      const overfillBodies = settledBodies.filter(b => (b.position.y - b.circleRadius) < fullLineY);
      const restedOverfillBodies = overfillBodies.filter(b => b.speed < 0.34);
      const xs = restedOverfillBodies.map(b => b.position.x);
      const overfillSpan = xs.length >= 2 ? (Math.max(...xs) - Math.min(...xs)) : 0;
    ...
    function rescueFloatingBodies() {
      ...
      for (const body of bodies) {
        const age = performance.now() - (body.spawnAt || 0);
        if (age < 900) continue;
        if (body.position.y < 96 || body.position.y > boardHeight - 88) continue;
        if (Math.abs(body.velocity.y) > 0.12 || Math.abs(body.angularVelocity || 0) > 0.04) continue;
        let hasSupportBelow = false;
        for (const other of bodies) {
          if (other.id === body.id) continue;
          const dx = Math.abs(other.position.x - body.position.x);
          const dy = other.position.y - body.position.y;
          const supportDx = body.circleRadius + other.circleRadius - 10;
          if (dx > supportDx || dy < 0) continue;
          if (dy <= body.circleRadius + other.circleRadius + 12) {
            hasSupportBelow = true;
            break;
          }
        }
        if (hasSupportBelow) continue;
        Body.translate(body, { x:0, y:1.5 });
        Body.setVelocity(body, { x: body.velocity.x * 0.45, y: Math.max(0.85, body.velocity.y) });
      }
    }
    ...
    function initEngine() {
      engine = Engine.create({ gravity:{ x:0, y:1 }, positionIterations:5, velocityIterations:4, constraintIterations:1 });
      render = null;
      updateWorldBounds();
    ...
    function dropAt(clientX) {
      ...
      const safeDrop = findSafeDropX(desiredX, radius);
      hoverX = safeDrop.x;
      renderPreview();
    ...
    function requestLoop(ts) {
      ...
      const chatInterval = state.clipTime > 0 ? 4.6 : 6.8;
      ...
      if (state.visualTimer >= (1 / 30)) {
        updateFx(state.visualTimer);
        syncBodyVisuals();
        state.visualTimer = 0;
      }
      if (state.uiTimer >= 0.05) {
        updateUi();
        state.uiTimer = 0;
      }
