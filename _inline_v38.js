  ...
  (() => {
    ...
    const BOARD_BODY_ITEM_SCALE = 1.5;
    ...
    function worldBodies() {
      ...
      const bodies = Composite.allBodies(engine.world).filter(b => !b.isStatic && !b.plugin?.pendingRemoval && (b.gameType === 'content' || b.gameType === 'hazard' || b.gameType === 'fire' || b.gameType === 'buzz'));
    ...
    function requestLoop(ts) {
      ...
      if (state.cleanupTimer >= 0.24) {
        state.cleanupTimer = 0;
        cleanupOffscreenBodies();
        purgeBrokenBodies();
      }
      if (state.rescueTimer >= 0.28) {
        state.rescueTimer = 0;
        rescueFloatingBodies();
      }
      if (state.visualTimer >= (1 / 20)) {
        updateFx(state.visualTimer);
        syncBodyVisuals();
        state.visualTimer = 0;
      }
      if (state.uiTimer >= 0.16) {
        updateUi();
        state.uiTimer = 0;
      }
